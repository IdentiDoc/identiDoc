...
class TestFileUploadHelpers(unittest.TestCase):
    def test_is_valid_filename(self):
        ...
        self.assertTrue(FileUpload.valid_filename('test.heic'))

        self.assertTrue(FileUpload.valid_filename('1.2.3.4.5.6.7.8.png'))
        self.assertTrue(FileUpload.valid_filename('a_really.long.filename.with.lotsofperiods.pdf'))

        self.assertFalse(FileUpload.valid_filename('test.txt'))
